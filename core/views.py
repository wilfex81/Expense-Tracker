...
def accounts(request):
    accounts = Account.objects.all().annotate(total_balance=Sum('deposited_amount'))
    if request.method == 'POST':
...
def transactions(request):
    ...
    if request.method == 'POST':
            ...
            transaction_date = datetime.strptime(transaction_date_str, '%B %d, %Y').date()
        ...
        if request.GET.get('category'):
            ...
            transactions = Transaction.objects.filter(budget_categorie__id=category_id)
